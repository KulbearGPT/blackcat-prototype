...
- [x] **M2-US-01：陪玩准入、标签、Presence 与可接单状态**
  ...
  - 进度记录（2026-07-17）：已完成 `@blackcat/api/players`、`InMemoryPlayerStore`、`PostgresPlayerStore`、`selectEligibleDispatchCandidates`、玩家工作台/本人 availability/Discord Presence sync/管理员审核/运营状态/标签管理 API、`buildApiServer({ player })` 和 runtime `PostgresPlayerStore` wiring。Bot 侧已新增 `HttpBotApiClient.syncDiscordPresence` 与 Sapphire `presence-update` listener，Presence 同步不会覆盖业务可接单开关。`npx vitest run tests/m2-us-01-db.spec.ts` 2/2 通过，runtime wiring RED/GREEN 单项通过，`npx vitest run tests/m2-us-01-api.spec.ts tests/m2-us-01-bot.spec.ts tests/m2-us-01-db.spec.ts` 3 files / 12 tests 通过，`npm run typecheck` 通过，`npm test` 25 files / 162 tests 通过。证据：`evidence/P0/M2-US-01/summary.md`。Discord credential 暂未提供，真实 Discord Server PresenceUpdate E2E 未执行。
